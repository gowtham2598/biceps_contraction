...
# 1. Define our physiologically accurate multipliers and folder paths
ascl_multipliers = ["0.5", "1.0", "1.5", "2.0"]
feb_input_dir = "feb_inputs/active_ascl"
log_dir = "raw_logs/active_ascl"
plot_dir = "febio_results/active_ascl"
...
print("="*70)
print("LAUNCHING AUTOMATED COUPLING LOOP: ACTIVE SCALE FACTOR SWEEP (ascl)")
print("="*70)

for m in ascl_multipliers:
    input_filename = f"biceps_ascl_{m}.feb"
    ...
    if process.returncode == 0:
        ...
        disp_file = os.path.join(log_dir, f"ascl_{m}_disp.txt")
        stress_file = os.path.join(log_dir, f"ascl_{m}_stress.txt")
        vol_file = os.path.join(log_dir, f"ascl_{m}_vol.txt")
