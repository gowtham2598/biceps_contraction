...
# 1. Define our calibrated multipliers and folder paths
camax_multipliers = ["0.0", "2.175", "4.35", "8.7", "17.4"]
feb_input_dir = "feb_inputs/active_camax"
log_dir = "raw_logs/active_camax"
plot_dir = "febio_results/active_camax"
...
print("="*70)
print("LAUNCHING AUTOMATED COUPLING LOOP: CALCIUM SENSITIVITY SWEEP (camax)")
print("="*70)

for m in camax_multipliers:
    input_filename = f"biceps_camax_{m}.feb"
    ...
    if process.returncode == 0:
        ...
        disp_file = os.path.join(log_dir, f"camax_{m}_disp.txt")
        stress_file = os.path.join(log_dir, f"camax_{m}_stress.txt")
        vol_file = os.path.join(log_dir, f"camax_{m}_vol.txt")
